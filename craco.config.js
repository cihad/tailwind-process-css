...
const moduleOverrides = {
	...
	"glob-parent": path.resolve(
		__dirname,
		"src/tailwind/modules/glob-parent.js"
	),
				...
				for (let i = 0; i < files.length; i++) {
					if (
						...
						return JSON.stringify(
							fs.readFileSync(files[i].file, "utf8")
						)
					}
				}
				return m
...
module.exports = {
	webpack: {
		...
		configure: (config, { env, paths }) => {
			config.output = {
				...(config.output || {}),
				filename: "__run__[name]_[fullhash].js",
				chunkFilename: "__run__[name].[chunkhash:20].esm.js",
			}
			...
			config.module.rules.push({
				test: require.resolve(
					"tailwindcss-v1/lib/plugins/preflight.js"
				),
			...
			config.module.rules.push({
				test: require.resolve(
					"tailwindcss-v2/lib/plugins/preflight.js"
				),
			...
			config.module.rules.push({
				test: require.resolve(
					"tailwindcss-insiders/lib/corePlugins.js"
				),
			...
			config.module.rules.push({
				...
				use: [
					createLoader(function (_source) {
						let agents =
							require("caniuse-lite/dist/unpacker/agents.js").agents

						for (let name in agents) {
							for (let key in agents[name]) {
								if (
									key !== "prefix" &&
									key !== "prefix_exceptions"
								) {
				...
				use: [
					createLoader(function (_source) {
						...
						for (let key in result) {
							result[key].browsers = result[key].browsers.filter(
								(b) => browsers.includes(b)
							)
